...
import { FRAGMENT_SRC, VERTEX_SRC } from './PixelBlast.shaders';
...
const SHAPE_MAP: Record<'square' | 'circle' | 'triangle' | 'diamond', number> = {
  square: 0,
  circle: 1,
  triangle: 2,
  diamond: 3,
...
const PixelBlast = ({
  ...
  const threeRef = useRef<PixelBlastThree | null>(null);
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((p) => {
            const effs = (p as unknown as { effects?: { uniforms?: Map<string, { value: number }> }[] }).effects;
            effs?.forEach((eff) => {
      ...
      if (t.liquidEffect) {
        const uStrengthUni = t.liquidEffect.uniforms.get('uStrength');
        if (uStrengthUni) uStrengthUni.value = liquidStrength;
